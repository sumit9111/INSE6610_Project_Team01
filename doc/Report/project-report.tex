...
iSmartAlarm database got extracted form ‘/data/iSA.common/databases/’ and then browsed by using DB Browser. According to manual of this device and regarding the status found in the TB\_IPUDiary table in thie database there are 4 different modes for the iSmartAlarm system including, Arm, Disarm, Panic and Home.\\
...
In the TB\_IPUDiary table we found a name by ‘pandadodu’ which was the operator who deactivated the alarm system shortly before the man entered the tabletting room at 10:34 on the day of the incident. The nickname 'pandadodu' is probably a combination of the first and last names of him which is D. Pandana.
\begin{figure}[H]
    \centering
